...
if metodo == "Usar Adubo Formulado":
    ...
    dose_ha = 0
    if nut_base == "Nitrogênio (N)" and f_n > 0:
        dose_ha = (valor_rec / f_n) * 100
    elif nut_base == "Fósforo (P)" and f_p > 0:
        dose_ha = (valor_rec / f_p) * 100
    elif nut_base == "Potássio (K)" and f_k > 0:
        dose_ha = (valor_rec / f_k) * 100
...
if st.button("🚀 Gerar PDF"):
    try:
        ...
        pdf.cell(180, 10, " IDENTIFICAÇÃO DA ÁREA".encode('latin-1', 'replace').decode('latin-1'), ln=True, fill=True)
        pdf.set_font("Arial", size=11)
        pdf.cell(180, 8, f" Talhão: {talhao}  |  Cultura: {cultura}".encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.cell(180, 8, f" Área Total: {tamanho_area} {tipo_medida}".encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.ln(5)
        
        # Calagem
        pdf.set_font("Arial", 'B', 13)
        pdf.set_text_color(34, 139, 34)
        pdf.cell(180, 10, "1. RECOMENDAÇÃO DE CALAGEM".encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.set_font("Arial", size=11)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(180, 7, f" Dose: {nc_ha:.2f} t/ha".encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.set_font("Arial", 'B', 11)
        pdf.set_fill_color(235, 245, 235)
        pdf.cell(180, 9, f" TOTAL PARA A ÁREA: {nc_total:.2f} Toneladas".encode('latin-1', 'replace').decode('latin-1'), ln=True, fill=True)
        pdf.ln(5)
        
        # NPK
        pdf.set_font("Arial", 'B', 13)
        pdf.set_text_color(34, 139, 34)
        pdf.cell(180, 10, "2. RECOMENDAÇÃO DE ADUBAÇÃO NPK".encode('latin-1', 'replace').decode('latin-1'), ln=True)
        pdf.set_font("Arial", size=11)
        pdf.set_text_color(0, 0, 0)
        pdf.multi_cell(180, 8, f" Detalhes: {detalhes_pdf}".encode('latin-1', 'replace').decode('latin-1'))
        
        if metodo == "Usar Adubo Formulado" and valor_final_kg > 0:
            pdf.ln(2)
            pdf.set_font("Arial", 'B', 12)
            pdf.set_fill_color(230, 245, 230)
            pdf.cell(180, 10, f" QUANTIDADE TOTAL: {valor_final_kg:.1f} kg ({sacos_calc} sacos)".encode('latin-1', 'replace').decode('latin-1'), ln=True, fill=True)
